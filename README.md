...
This project uses Python to analyze species and observation records associated with four United States national parks. It examines species composition, recorded conservation statuses, category-level conservation rates, and observation patterns while placing particular emphasis on data quality and safe dataset integration.
...
> **Important:** The dataset is inspired by National Park Service information but is mostly fictional. Results are appropriate for demonstrating analytical methods and should not be used to make real conservation decisions.
...
- Yellowstone has the largest recorded observation total at **1,443,562**, but park size and survey effort are unavailable, so this should not be interpreted as a population or biodiversity ranking.
...
4. Aggregated observation records to one row per scientific-name and park combination.
...
2. Place both CSV files in the repository root.
...
The notebook should use a relative project path so it works on another computer:
